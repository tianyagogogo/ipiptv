...
cd /root/ipiptv && git config --system user.email "24786545+tianyagogogo@users.noreply.github.com" && git config --system user.name "tianyagogogo"  && git pull && git add -A  && git commit -a -m 'git commit '  && git push origin main
